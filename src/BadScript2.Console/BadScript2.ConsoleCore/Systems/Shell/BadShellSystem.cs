...
public class BadShell
{
    ...
    private static string ConfigDirectory => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments, Environment.SpecialFolderOption.Create), ".badshell");
    ...
    private void LoadUserSettings()
    {
        ...
        {
            Dictionary<string, string> env = JsonConvert.DeserializeObject<Dictionary<string, string>>(File.ReadAllText(envPath))!;
    ...
    private BadTask RunScript(string script)
    {
        BadTask task = new BadTask(
            new BadInteropRunnable(
                m_Context!.Execute(
                        BadSourceParser.Parse("<stdin>", script)
                    )
                    .GetEnumerator(),
                true
            ),
    ...
    private BadTask RunCommand(string file, string[] args)
    {
        ...
        BadTask task = new BadTask(new BadInteropRunnable(WaitFor(proc).GetEnumerator(), true), file + " " + string.Join(" ", args));
        m_TaskRunner.AddTask(task, true);
