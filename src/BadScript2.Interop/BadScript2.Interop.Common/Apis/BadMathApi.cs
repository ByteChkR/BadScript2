...
public class BadMathApi : BadInteropApi
{
	...
	protected override void LoadApi(BadTable target)
	{
		...
		target.SetFunction<decimal>("Floor",
		...
		target.SetFunction<decimal>("Log10",
			x => (decimal)Math.Log10((double)x),
			BadNativeClassBuilder.GetNative("num"));
		target.SetFunction<decimal, decimal>("Max",
			(x, y) => (decimal)Math.Max((double)x, (double)y),
			BadNativeClassBuilder.GetNative("num"));
		target.SetFunction<decimal, decimal>("Min",
			(x, y) => (decimal)Math.Min((double)x, (double)y),
			BadNativeClassBuilder.GetNative("num"));
		target.SetFunction<decimal, decimal>("Pow",
			(x, y) => (decimal)Math.Pow((double)x, (double)y),
			BadNativeClassBuilder.GetNative("num"));
		target.SetFunction<decimal, decimal>("Round",
		...
		target.SetFunction<decimal>("Truncate",
