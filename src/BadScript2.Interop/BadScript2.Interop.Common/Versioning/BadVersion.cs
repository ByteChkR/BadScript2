...
{
    public static readonly BadClassPrototype Prototype = new BadNativeClassPrototype<BadVersion>("Version", VersionCtor);
    ...
    public BadVersion(Version version)
    {
        ...
        m_ChangeVersion = BadObjectReference.Make(
            ...
            () => new BadDynamicInteropFunction<string>("ChangeVersion", (ctx, s) => new BadVersion(m_Version.ChangeVersion(s)))
        );
    }
    ...
    {
        return propName is IBadString str && (str.Value == "Major" || str.Value == "Minor" || str.Value == "Build" || str.Value == "Revision") || base.HasProperty(propName);
    }
    ...
    private static BadObject VersionCtor(BadExecutionContext ctx, BadObject[] args)
    {
        ...
        if (args.Length >= 2)
        {
            ...
            if (args.Length == 4)
            {
                return new BadVersion(new Version((int)major.Value, (int)minor.Value, (int)build.Value, (int)revision.Value));
            }
        }
