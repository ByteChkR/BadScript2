...
internal partial class BadZipApi
{
    ...
    private void ToDirectoryApi([BadParameter(description: "Output Directory")] string outputDir,
                                [BadParameter(description: "Input File")] string inputFile)
    {
    ...
    private void FromDirectoryApi([BadParameter(description: "Input Directory")] string inputDir,
                                  [BadParameter(description: "Output File")] string outputFile)
    {
    ...
    private void FromDirectory(string inputDir, Stream output)
    {
        ...
        string[] files = m_FileSystem.GetFiles(inputDir, "", true)
                                     .ToArray();

        foreach (string file in files)
        {
            string zipPath = file.Remove(0, inputDir.Length + 1)
                                 .Replace('\\', '/');
