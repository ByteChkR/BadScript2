...
public class BadLinqExtensions : BadInteropExtension
{
    ...
    {
        var o = BadObject.Null;

        foreach (var obj in func.Invoke(args, ctx))
        {
            o = obj;
        }
    ...
    private static BadObject FirstIndex(BadExecutionContext ctx, IBadEnumerable e, BadObject o)
    {
        if (o == BadObject.Null)
        {
            return e.FirstOrDefault() == null ? -1 : 0;
        }

        if (o is BadFunction filter)
        {
            //Find the first element that matches the filter and return its index
            var index = 0;
            foreach (var obj in e)
            {
                if (Invoke<bool>(filter, ctx, obj))
                {
                    return index;
                }
    ...
    private static BadObject LastIndex(BadExecutionContext ctx, IBadEnumerable e, BadObject o)
    {
        var filter = o as BadFunction;
        //Find the last element that matches the filter and return its index
        var index = 0;
        var lastIndex = -1;
        foreach (var obj in e)
        {
            if (filter == null || Invoke<bool>(filter, ctx, obj))
            {
                lastIndex = index;
            }
    ...
    private static BadObject Min(BadExecutionContext ctx, IBadEnumerable e, BadObject predicate)
    {
        if (predicate == BadObject.Null)
        {
            return e.Min();
        ...
        if (predicate is BadFunction func)
        {
            return e.Min(x => Invoke<bool>(func, ctx, x));
        }

        throw new BadRuntimeException("Invalid Predicate");
    }

    public static BadObject Max(BadExecutionContext ctx, IBadEnumerable e, BadObject predicate)
    {
        if (predicate == BadObject.Null)
        {
            return e.Max();
        ...
        if (predicate is BadFunction func)
        {
            return e.Max(x => Invoke<bool>(func, ctx, x));
        }
    ...
    protected override void AddExtensions(BadInteropExtensionProvider provider)
    {
        ...
        provider.RegisterObject<IBadEnumerable>("FirstIndex",
            e => new BadInteropFunction("FirstIndex",
                (c, args) => FirstIndex(c, e, args[0]),
                false,
                BadNativeClassBuilder.GetNative("num"),
                new BadFunctionParameter("selector",
                    true,
        ...
        provider.RegisterObject<IBadEnumerable>("LastIndex",
            e => new BadInteropFunction("LastIndex",
                (c, args) => LastIndex(c, e, args[0]),
                false,
                BadNativeClassBuilder.GetNative("num"),
                new BadFunctionParameter("selector",
                    true,
        ...
        provider.RegisterObject<IBadEnumerable>("Any",
            e => new BadInteropFunction("Any",
                (c, args) => Any(c, e, args.Length == 0 ? BadObject.Null : args[0]),
                false,
        ...
        provider.RegisterObject<IBadEnumerable>("Min",
            e => new BadInteropFunction("Min",
                (c, args) => Min(c, e, args.Length == 0 ? BadObject.Null : args[0]),
                false,
                BadNativeClassBuilder.GetNative("num"),
                new BadFunctionParameter("predicate",
                    true,
        ...
        provider.RegisterObject<IBadEnumerable>("Max",
            e => new BadInteropFunction("Max",
                (c, args) => Max(c, e, args.Length == 0 ? BadObject.Null : args[0]),
                false,
                BadNativeClassBuilder.GetNative("num"),
                new BadFunctionParameter("predicate",
                    true,
        ...
        return new BadInteropEnumerable(e.OrderBy(o =>
                {
                    ...
                    foreach (var o1 in function.Invoke(new[] { o },
                                 ctx
                             ))
                    {
                        r = o1;
        ...
        return new BadInteropEnumerable(ordered.ThenBy(o =>
                {
                    ...
                    foreach (var o1 in function.Invoke([o],
                                 ctx
                             ))
                    {
                        r = o1;
