using BadScript2.Parser.Expressions;
using BadScript2.Parser.Expressions.Block.Loop;
using BadScript2.Runtime.Objects;

namespace BadScript2.Runtime.Compiler.Expression.Block;

public class BadWhileExpressionCompiler : BadExpressionCompiler<BadWhileExpression>
{
    public override int Compile(BadWhileExpression expr, BadCompilerResult result)
    {
        int condition = BadCompiler.CompileExpression(expr.Condition, result);
        int falseJump = result.Emit(new BadInstruction(BadOpCode.JumpIfFalse, expr.Condition.Position, BadObject.Null));

        result.Emit(
            new BadInstruction(
                BadOpCode.CreateScope,
                expr.Position,
                "WhileLoop",
                (decimal)(BadScopeFlags.Breakable | BadScopeFlags.Continuable)
            )
        );
        foreach (BadExpression expression in expr.Body)
        {
            BadCompiler.CompileExpression(expression, result);
        }

        result.Emit(new BadInstruction(BadOpCode.DestroyScope, expr.Position));

        result.Emit(new BadInstruction(BadOpCode.Jump, expr.Position, condition));
        int end = result.Emit(new BadInstruction(BadOpCode.Nop, expr.Position));
        result.SetArgument(falseJump, 0, end);

        return condition;
    }
}