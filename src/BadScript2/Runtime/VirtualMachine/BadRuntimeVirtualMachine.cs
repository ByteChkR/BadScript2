        ...
        while (m_InstructionPointer < m_Instructions.Length)
        {
            ...
            switch (instr.OpCode)
            {
                ...
                case BadOpCode.AquireLock:
                {
                    ...
                    {
                        throw new BadRuntimeException("Lock object must be of type Array, Object or Class", instr.Position);
                    }
                ...
                case BadOpCode.HasProperty:
                {
                    ...
                    m_ArgumentStack.Push(obj.HasProperty(key));
                ...
                case BadOpCode.Range:
                {
                    ...
                    m_ArgumentStack.Push(new BadInteropEnumerator(BadRangeExpression.Range(sn.Value, en.Value).GetEnumerator()));
                ...
                case BadOpCode.LoadMember:
                    m_ArgumentStack.Push(m_ArgumentStack.Pop().Dereference().GetProperty((string)instr.Arguments[0], ctx.Scope));
                ...
                case BadOpCode.Not:
                {
                    m_ArgumentStack.Push(((IBadBoolean)m_ArgumentStack.Pop().Dereference()).Value ? BadObject.False : BadObject.True);
                ...
                case BadOpCode.Add:
                {
                    BadObject right = m_ArgumentStack.Pop().Dereference();
                    BadObject left = m_ArgumentStack.Pop().Dereference();
                    m_ArgumentStack.Push(BadAddExpression.Add(left, right, instr.Position));
                ...
                case BadOpCode.Sub:
                {
                    BadObject right = m_ArgumentStack.Pop().Dereference();
                    BadObject left = m_ArgumentStack.Pop().Dereference();
                    m_ArgumentStack.Push(BadSubtractExpression.Sub(left, right, instr.Position));
                ...
                case BadOpCode.Mul:
                {
                    BadObject right = m_ArgumentStack.Pop().Dereference();
                    BadObject left = m_ArgumentStack.Pop().Dereference();
                    m_ArgumentStack.Push(BadMultiplyExpression.Mul(left, right, instr.Position));
                ...
                case BadOpCode.Div:
                {
                    BadObject right = m_ArgumentStack.Pop().Dereference();
                    BadObject left = m_ArgumentStack.Pop().Dereference();
                    m_ArgumentStack.Push(BadDivideExpression.Div(left, right, instr.Position));
                ...
                case BadOpCode.Mod:
                {
                    BadObject right = m_ArgumentStack.Pop().Dereference();
                    BadObject left = m_ArgumentStack.Pop().Dereference();
                    m_ArgumentStack.Push(BadModulusExpression.Mod(left, right, instr.Position));
                ...
                case BadOpCode.Equals:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadEqualityExpression.EqualWithOverride(ctx, left, right, instr.Position))
                        {
                            obj = o;
                ...
                case BadOpCode.NotEquals:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadInequalityExpression.NotEqualWithOverride(ctx, left, right, instr.Position))
                        {
                            obj = o;
                ...
                case BadOpCode.Greater:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadGreaterThanExpression.GreaterThanWithOverride(ctx, left, right, instr.Position))
                        {
                            obj = o;
                ...
                case BadOpCode.GreaterEquals:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadGreaterOrEqualExpression.GreaterOrEqualWithOverride(ctx, left, right, instr.Position))
                        {
                            obj = o;
                ...
                case BadOpCode.Less:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadLessThanExpression.LessThanWithOverride(ctx, left, right, instr.Position))
                        {
                            obj = o;
                ...
                case BadOpCode.LessEquals:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadLessOrEqualExpression.LessOrEqualWithOverride(ctx, left, right, instr.Position))
                        {
                            obj = o;
                ...
                case BadOpCode.AddAssign:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadAddAssignExpression.AddWithOverride(ctx, left, right, instr.Position, "+="))
                        {
                            obj = o;
                ...
                case BadOpCode.SubAssign:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadSubtractAssignExpression.SubtractWithOverride(ctx, left, right, instr.Position, "-="))
                        {
                            obj = o;
                    ...
                    {
                        obj = BadSubtractAssignExpression.Subtract(left, left.Dereference(), right, instr.Position, "-=");
                    }
                ...
                case BadOpCode.MulAssign:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadMultiplyAssignExpression.MultiplyWithOverride(ctx, left, right, instr.Position, "*="))
                        {
                            obj = o;
                    ...
                    {
                        obj = BadMultiplyAssignExpression.Multiply(left, left.Dereference(), right, instr.Position, "*=");
                    }
                ...
                case BadOpCode.DivAssign:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadDivideAssignExpression.DivideWithOverride(ctx, left, right, instr.Position, "/="))
                        {
                            obj = o;
                ...
                case BadOpCode.ModAssign:
                {
                    ...
                    if (m_UseOverrides)
                    {
                        foreach (BadObject o in BadModulusAssignExpression.ModulusWithOverride(ctx, left, right, instr.Position, "%="))
                        {
                            obj = o;
                ...
                case BadOpCode.PostInc:
                {
                    ...
                    m_ArgumentStack.Push(BadPostIncrementExpression.Increment(obj, instr.Position));
                ...
                {
                    BadObjectReference obj = (BadObjectReference)m_ArgumentStack.Pop();
                    m_ArgumentStack.Push(BadPostDecrementExpression.Decrement(obj, instr.Position));
                ...
                {
                    BadObjectReference obj = (BadObjectReference)m_ArgumentStack.Pop();
                    m_ArgumentStack.Push(BadPreIncrementExpression.Increment(obj, instr.Position));
                ...
                {
                    BadObjectReference obj = (BadObjectReference)m_ArgumentStack.Pop();
                    m_ArgumentStack.Push(BadPreDecrementExpression.Decrement(obj, instr.Position));
