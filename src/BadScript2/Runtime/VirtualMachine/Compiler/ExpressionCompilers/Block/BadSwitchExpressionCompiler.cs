...
public class BadSwitchExpressionCompiler : BadExpressionCompiler<BadSwitchExpression>
{
    public override void Compile(BadExpressionCompileContext context, BadSwitchExpression expression)
    {
        ...
        List<int>
            caseEndJumps = new List<int>(); //List of jumps to the end of the switch statement(after the default case)
        ...
        foreach (KeyValuePair<BadExpression, BadExpression[]> @case in expression.Cases)
        {
            ...
            {
                int endJump =
                    context.EmitEmpty(); //Jump to the next case if the case value is not equal to the switch value

                //We have a case body,
                //Resolve all caseBodyJumps to the start of the case body
                foreach (int caseBodyJump in caseBodyJumps)
                {
                    context.ResolveEmpty(caseBodyJump,
                                         BadOpCode.JumpRelativeIfTrue,
                                         expression.Position,
                                         context.InstructionCount - caseBodyJump - 1
                                        );
                }
        ...
        {
            context.ResolveEmpty(caseEndJump,
                                 BadOpCode.JumpRelative,
                                 expression.Position,
                                 context.InstructionCount - caseEndJump - 1
                                );
        }
        ...
        context.ResolveEmpty(setBreakInstruction,
                             ...
                             expression.Position,
                             context.InstructionCount - switchScopeStart
                            );
