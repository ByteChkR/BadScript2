...
public class BadWhileExpressionCompiler : BadExpressionCompiler<BadWhileExpression>
{
	public override IEnumerable<BadInstruction> Compile(BadCompiler compiler, BadWhileExpression expression)
	{
		...
		instructions.Add(new BadInstruction(BadOpCode.CreateScope,
			expression.Position,
			"WhileScope",
			BadObject.Null,
		...
		instructions[endJump] = new BadInstruction(BadOpCode.JumpRelativeIfFalse,
			expression.Position,
			instructions.Count - endJump);

		//address to the end of the loop(relative to the create scope instruction)
		instructions[setBreakInstruction] = new BadInstruction(BadOpCode.SetBreakPointer,
			expression.Position,
			instructions.Count - loopScopeStart - 1);

		//Address to start of the condition(relative to the start of the loop)
		instructions[setContinueInstruction] = new BadInstruction(BadOpCode.SetContinuePointer,
			expression.Position,
