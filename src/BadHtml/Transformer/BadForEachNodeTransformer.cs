...
public class BadForEachNodeTransformer : BadHtmlNodeTransformer
{
    ...
    public override void TransformNode(BadHtmlContext context)
    {
        ...
        {
            throw BadRuntimeException.Create(context.ExecutionContext.Scope, "Missing 'on' attribute in 'bs:each' node", context.CreateOuterPosition());
        }

        if (itemAttribute == null)
        {
            throw BadRuntimeException.Create(context.ExecutionContext.Scope, "Missing 'as' attribute in 'bs:each' node", context.CreateOuterPosition());
        }

        if (string.IsNullOrEmpty(enumerationAttribute.Value))
        {
            throw BadRuntimeException.Create(context.ExecutionContext.Scope, "Empty 'on' attribute in 'bs:each' node", context.CreateAttributePosition(enumerationAttribute));
        }

        if (string.IsNullOrEmpty(itemAttribute.Value))
        {
            throw BadRuntimeException.Create(context.ExecutionContext.Scope, "Empty 'as' attribute in 'bs:each' node", context.CreateAttributePosition(itemAttribute));
        }

        BadObject enumeration = context.ParseAndExecute(enumerationAttribute.Value, context.CreateAttributePosition(enumerationAttribute));
        ...
        else if (enumeration is IBadEnumerator badEnumerator)
        {
            while (badEnumerator.MoveNext())
            {
                RunIteration(
                    ...
                    throw BadRuntimeException.Create(context.ExecutionContext.Scope, "Enumerator returned null", context.CreateAttributePosition(enumerationAttribute))
                );
            }
        }
