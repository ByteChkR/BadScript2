...
public class BadTryCatchExpressionCompiler : BadExpressionCompiler<BadTryCatchExpression>
{
    public override IEnumerable<BadInstruction> Compile(BadCompiler compiler, BadTryCatchExpression expression)
    {
        ...
        instructions.Add(new BadInstruction(BadOpCode.CreateScope, expression.Position, "TryScope", BadObject.Null, BadScopeFlags.CaptureThrow));
        ...
        instructions.Add(new BadInstruction(BadOpCode.DefVar, expression.Position, (BadObject)expression.ErrorName, true));
        ...
        instructions.Add(new BadInstruction(BadOpCode.DestroyScope, expression.Position));
        instructions[setThrowInstruction] = new BadInstruction(BadOpCode.SetThrowPointer, expression.Position, catchStart - 1);
        instructions[jumpToEnd] = new BadInstruction(BadOpCode.JumpRelative, expression.Position, instructions.Count - jumpToEnd - 1);
